...
- [ ] 写失败测试：默认设置已初始化且为普通用户；旧程序员快照解码后迁移为普通用户并保留自定义文字、主题和音色。
- [ ] 构建并运行 Hypium，确认测试因仍返回程序员和未初始化而失败。
- [ ] 修改默认值和解码迁移，删除 Index 的首启分支及设置页用户模式区域。
- [ ] 删除首页模式标签和顶部自定义文字，仅保留“今日功德 + 数值”。
- [ ] 运行 Hypium 与 debug 构建，提交 `统一普通用户并精简首页信息`。
...
- [ ] 写失败测试：自动档运行后切换自由档会停止定时器；再次选择自动档会回到自由；目标完成后 `targetCompleted=true` 且额外敲击无效。
- [ ] 运行 Hypium，确认失败来自缺少目标完成状态和档位切换 API。
- [ ] 在 `SessionService.selectMode()` 中集中处理自由、目标、自动及同档自动关闭，删除限时路径。
- [ ] 使用 `session.mode` 渲染选中态，目标档显示进度和完成卡。
- [ ] 运行 Hypium 与 debug 构建，提交 `修复会话互斥并补充目标完成反馈`。
...
- [ ] 写失败测试：固定记录在日、周、月、年口径下得到不同摘要、标题、桶数和标签。
- [ ] 运行 Hypium，确认因 `StatsPeriodService` 尚不存在而编译失败。
- [ ] 实现 `StatsPeriodView`、`StatsBucket` 和纯逻辑聚合服务。
- [ ] 仓库暴露全部记录副本，AppStore 同步全量记录，统计页按选中周期实时生成视图。
- [ ] 图表改用桶标签和值，主题标题随周期变化。
- [ ] 运行 Hypium 与 debug 构建，提交 `实现日周月年统计聚合`。
...
- [ ] 写失败测试：浅色、深色和跟随系统在系统明暗值下解析出正确结果。
- [ ] 运行 Hypium，确认因 `ThemeResolver` 尚不存在而编译失败。
- [ ] EntryAbility 把系统色彩模式写入 AppStorage，Index 解析最终 `dark`。
- [ ] 所有页面、统计图和底栏接收 `dark` 并切换背景、卡片、主次文字与分隔线。
- [ ] 设置页音色行打开三项选择面板，点击保存并调用 SoundPool 试听。
- [ ] 运行 Hypium、debug 构建和模拟器点击验证，提交 `实现完整主题与音色选择`。
...
- [ ] 写失败测试：每次手动或自动记录都把 `hitSequence` 精确加一。
- [ ] 运行 Hypium，确认缺少 `hitSequence` 而失败。
- [ ] 从用户原木鱼素材编辑出仅移除静态木槌的 `muyu_body.png`，验证木鱼纹理和构图保持。
- [ ] WoodenFishView 观察 `hitSequence`，统一触发木槌落下、木鱼回弹和无次数尾缀的浮动文字。
- [ ] 模拟器验证手动与自动动画、自动切档停止和目标完成。
- [ ] 运行 Hypium、静态门禁、debug/release 构建，提交 `重做木槌敲击动画并完成改版验收`。
...
- [ ] 在 phone 模拟器记录首页、自动运行、目标完成、四种统计、音色面板、浅色和深色截图。
- [ ] 更新功能说明、验收结果、HAP 大小和已知设备限制。
- [ ] 执行 `scripts/check-standard.ps1`、外部标准检查、Hypium、debug/release 构建和 `git diff --check`。
- [ ] 确认工作区只包含本轮改动，提交 `记录反馈改版完整验收结果`。
